    ...
    <AppLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="heading-primary">Product Health & Return Intelligence</h1>
          ...
          <div className="flex items-center space-x-3 text-sm text-slate-600">
            <span className="px-4 py-2 rounded-xl bg-white border border-slate-400 shadow-sm font-mono text-sm font-extrabold">
              Total Products Tracked: <strong className="text-indigo-600 font-black">{productMetrics.length}</strong>
            </span>
          </div>
        </div>

        {/* Toolbar & Filters */}
        <div className="glass-panel p-5 rounded-2xl border border-slate-400 flex flex-col md:flex-row items-center justify-between gap-4">
          {/* Search Bar */}
          <div className="relative w-full md:w-96">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-100/90 border border-slate-400 rounded-xl text-sm text-slate-900 placeholder-slate-500 focus:outline-none focus:bg-white focus:border-indigo-600 transition font-medium"
            />
          </div>

          {/* Filter Selects */}
          <div className="flex items-center space-x-3.5 w-full md:w-auto justify-end text-sm">
            <div className="flex items-center space-x-2">
              <Filter className="w-4 h-4 text-slate-500" />
              <span className="text-slate-700 font-extrabold">Category:</span>
              <select
            ...
            <div className="flex items-center space-x-2">
              <span className="text-slate-700 font-extrabold">Risk Level:</span>
              <select
                value={selectedRisk}
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProducts.map((prod) => (
              <div
                key={prod.sku}
                className="glass-panel p-6 rounded-3xl border border-slate-400 hover:border-indigo-500 transition-all group flex flex-col justify-between space-y-6 shadow-sm"
              >
                {/* Header: Name, SKU, Category, Risk Level Badge */}
                <div className="space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h3 className="font-extrabold text-lg text-slate-900 group-hover:text-indigo-600 transition line-clamp-1">
                        {prod.productName}
                      </h3>
                      <div className="flex items-center space-x-2.5 mt-1.5">
                        <span className="px-2.5 py-0.5 rounded-lg bg-indigo-50 text-indigo-700 border border-indigo-300 text-xs font-mono font-extrabold">
                          {prod.sku}
                        </span>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  {/* Total Returns */}
                  <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-300 space-y-1">
                    <span className="text-xs text-slate-600 uppercase font-extrabold">Total Returns</span>
                    <p className="text-xl font-black text-slate-900">{prod.totalReturns}</p>
                  </div>

                  {/* High Severity Count */}
                  <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-300 space-y-1">
                    <span className="text-xs text-slate-600 uppercase font-extrabold">High Severity</span>
                    <p className={`text-xl font-black ${prod.highSeverityCount > 0 ? 'text-rose-600' : 'text-slate-800'}`}>
                ...
                <div className="p-4 rounded-2xl bg-slate-50 border border-slate-300 text-sm space-y-2">
                  <div className="flex justify-between items-center text-xs sm:text-sm">
                    <span className="text-slate-600 font-bold">Most Common Reason</span>
                    <span className="font-extrabold text-indigo-600">{prod.topReasonPercentage}% of returns</span>
                  </div>

                  <p className="font-extrabold text-slate-900 truncate text-sm sm:text-base">{prod.mostCommonReturnReason}</p>
                ...
                <div className="pt-2 border-t border-slate-300">
                  <Link
                    ...
                    className="w-full flex items-center justify-center space-x-2 py-3 px-4 rounded-2xl bg-indigo-50 hover:bg-indigo-600 text-indigo-800 hover:text-white border border-indigo-300 text-sm font-extrabold shadow-sm transition duration-200"
                  >
